...
const UpdateCategoriesPages = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(`http://localhost:8000/api/categories/findCategoryPage/${id}`);
  ...
  const updateData = async (updatedData) => {
    try {
      ...
      const response = await axios.put(`http://localhost:8000/api/categories/editCategoryPage/${id}`, updatedData, {
        headers: {
